...
Clarat::Application.routes.draw do
  ...
  localized do
    ...
    # TODO: remove this redirects after google saved them AND remove the locale
    # entries (routes) that were automatically generated by localeapp
    # get "/fr/*path/offres%20d%27aide/:slug", to: redirect('/fr/%{path}/offres-d-aide/%{slug}')
    # get("/fr/*path/offres%20d'aide", to: redirect do |_params, request|
    #   "/fr/#{request.params[:path]}/offres-d-aide?#{request.params.select { |key, _value| %(utf8 search_form).include?(key) }.to_query}"
    # end)
    # get('/fr/*path/listedeservices', to: redirect do |_params, request|
    #   "/fr/#{request.params[:path]}/offres-d-aide?#{request.params.select { |key, _value| %(utf8 search_form).include?(key) }.to_query}"
    # end)
