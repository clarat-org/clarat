...
Clarat::Application.routes.draw do
  ...
  localized do
    ...
    get '/fr/*path/liste-de-services/:slug', to: redirect('/fr/%<path>s/offres-d-aide/%<slug>s')
    get '/fr/*path/liste%20de%20services/:slug', to: redirect('/fr/%<path>s/offres-d-aide/%<slug>s')
    get('/fr/*path/liste-de-services', to: redirect do |_params, request|
      "/fr/#{request.params[:path]}/offres-d-aide?#{request.params.select { |key, _value| %(utf8 search_form).include?(key) }.to_query}"
    end)
    get('/fr/*path/liste%20de%20services', to: redirect do |_params, request|
      "/fr/#{request.params[:path]}/offres-d-aide?#{request.params.select { |key, _value| %(utf8 search_form).include?(key) }.to_query}"
    end)
